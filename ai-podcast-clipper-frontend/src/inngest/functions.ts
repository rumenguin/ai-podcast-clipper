...
export const processVideo = inngest.createFunction(
  ...
  async ({event, step}) => {
    ...
    try {
      ...
      return {userId: uploadedFile.user.id, credits: uploadedFile.user.credits, s3Key: uploadedFile.s3Key}
    });
    ...
    const { clipsFound } = await step.run("create-clips-in-db", async () => {
      ...
      const allKeys = await list3ObjectsByPrefix(folderPrefix);
...
async function list3ObjectsByPrefix(prefix: string) {
